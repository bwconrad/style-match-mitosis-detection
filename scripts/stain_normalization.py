...
def normalizeStaining(img, saveFile=None, Io=240, alpha=1, beta=0.15):
    ...
    # H[H>255] = 254
    # H = np.reshape(H.T, (h, w, 4)).astype(np.uint8)
    #
    # E = np.multiply(Io, np.exp(np.expand_dims(-HERef[:,1], axis=1).dot(np.expand_dims(C2[1,:], axis=0))))
    # E[E>255] = 254
    # E = np.reshape(E.T, (h, w, 4)).astype(np.uint8)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", type=Path, required=True, help="Input directory"
    )
    parser.add_argument(
    ...
    # Normalize each image in input directory
    ...
    for file in files:
        ...
        normalizeStaining(
            img=img, saveFile=output_path, Io=args.Io, alpha=args.alpha, beta=args.beta
        )
